...
export default function AdminDashboard() {
    ...
    // Simple trend data - show inquiries by day of month
    const trendDataMap: Record<string, number> = {};
    inquiries.forEach((inq: any) => {
        const date = new Date(inq.createdAt).toLocaleDateString('en-US', { day: 'numeric', month: 'short' });
        trendDataMap[date] = (trendDataMap[date] || 0) + 1;
    });
    const trendData = Object.entries(trendDataMap).map(([date, count]) => ({ date, count })).slice(-7); // Last 7 unique days
