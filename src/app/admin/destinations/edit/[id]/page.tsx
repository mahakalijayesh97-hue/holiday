...
import { MapPin, Globe, Mountain, Save, X, Plus, Trash2, ArrowLeft } from 'lucide-react';
...
export default function EditDestination() {
  ...
  useEffect(() => {
    ...
    if (id && status === 'authenticated') fetchDestination();
  }, [id, router, status]);
  ...
  const handleHighlightChange = (index: number, value: string) => {
    ...
    setFormData((prev: any) => ({ ...prev, highlights: newHighlights }));
  };

  const addHighlight = () => {
    setFormData((prev: any) => ({ ...prev, highlights: [...prev.highlights, ''] }));
  };
  ...
  const handleHotelChange = (category: 'budget' | 'comfort' | 'luxury', index: number, field: string, value: string) => {
    ...
    newHotels[category][index][field] = value;
    setFormData((prev: any) => ({ ...prev, hotels: newHotels }));
  };
  ...
  const removeHotel = (category: 'budget' | 'comfort' | 'luxury', index: number) => {
    const newHotels = { ...formData.hotels };
    newHotels[category] = newHotels[category].filter((_: any, i: number) => i !== index);
    setFormData((prev: any) => ({ ...prev, hotels: newHotels }));
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        toast.error(data.error || 'Failed to update destination');
      }
    } catch (error) {
  ...
  if (loading) return <LoadingSpinner text="Pulling record from archives..." />;
  if (!formData) return <div className="p-20 text-center">Record not found</div>;
    ...
    <main className="min-h-screen bg-gray-950 text-white pb-20">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-10">
          <div>
            <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-500 hover:text-white mb-4 text-xs font-bold uppercase tracking-widest">
              <ArrowLeft className="w-4 h-4" /> Back to Catalog
            </button>
            <h1 className="text-3xl font-extrabold flex items-center gap-3 tracking-tighter uppercase leading-none">
              Edit Registry <span className="text-blue-500 text-sm font-mono tracking-tighter px-2 py-1 bg-blue-500/10 rounded-lg">ID: {formData.name}</span>
            </h1>
          </div>
          <button
            onClick={() => router.back()}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className="glass p-8 rounded-3xl border-gray-800/50 space-y-6">
            <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-2">
              <Plus className="w-4 h-4 text-blue-400" /> Core Information
            </h3>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  readOnly
                  className="w-full bg-gray-900/50 border border-gray-800 rounded-2xl px-5 py-4 text-sm text-gray-500 cursor-not-allowed outline-none"
                  value={formData.name}
                />
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-gray-900 border border-gray-800 rounded-2xl px-5 py-4 text-sm focus:border-blue-500 outline-none transition-all"
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                ...
                <select
                  name="type"
                  className="w-full bg-gray-900 border border-gray-800 rounded-2xl px-5 py-4 text-sm focus:border-blue-500 outline-none transition-all appearance-none cursor-pointer"
                  value={formData.type}
                  onChange={handleInputChange}
                >
            ...
            {(['budget', 'comfort', 'luxury'] as const).map((category) => (
              <div key={category} className="space-y-4 pt-4 first:pt-0">
                ...
                <div className="space-y-3">
                  {(formData.hotels[category] || []).map((hotel: any, index: number) => (
          ...
          <div className="flex items-center justify-end gap-4 pt-6 pb-20">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-10 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-blue-900/30 transition-all hover:-translate-y-1 active:scale-95 flex items-center gap-3 disabled:opacity-50 disabled:translate-y-0"
            >
              {saving ? 'Saving changes...' : <><Save className="w-4 h-4" /> Update Registry Entry</>}
            </button>
          </div>
        </form>
      </div>
